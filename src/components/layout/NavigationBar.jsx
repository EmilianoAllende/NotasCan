...
const NavigationBar = ({ activeView, setActiveView, selectedOrg }) => {
  ...
  return (
    // Borde inferior eliminado para "pegarlo" al contenido
    <div className="bg-white dark:bg-gray-800">
      {/* Padding vertical (py) aumentado a py-2 para hacer la barra más alta */}
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150 ease-in-out disabled:opacity-40 disabled:cursor-not-allowed ${
              activeView === tab.id
                ? 'bg-gray-100 text-gray-900 dark:bg-gray-700 dark:text-gray-100' // Estilo Activo (fondo)
                : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 hover:text-gray-700 dark:hover:bg-gray-700/50 dark:hover:text-gray-200' // Estilo Inactivo (transparente)
            }`}
          >
            <tab.icon size={16} />
            {tab.label}
