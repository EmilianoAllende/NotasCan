...
import { Users, Eye, Mail, Building, FilePenLine, Database, LogOut, ShieldCheck, ChevronLeft, ChevronRight } from 'lucide-react';
import ThemeSwitcher from './ThemeSwitcher'; // Importamos el toggle

const Sidebar = ({ activeView, setActiveView, selectedOrg, onLogout, currentUser, isCollapsed, onToggle }) => {
  ...
  return (
    // --- ESTILO ACTUALIZADO: Ancho dinámico y 'relative' para el botón ---
    <aside className={`relative flex-shrink-0 flex flex-col bg-slate-50 dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 transition-all duration-300 ease-in-out ${
      ...
      <div className="h-16 flex items-center justify-center px-4 border-b border-slate-200 dark:border-slate-700">
        <Database size={20} className="text-blue-600 dark:text-blue-400 flex-shrink-0" />
        {/* --- ¡NUEVO! Título condicional --- */}
        {!isCollapsed && (
        ...
        {tabsToShow.map((tab) => {
          ...
          return (
            <button
              ...
              disabled={isDisabled}
              // --- ESTILO ACTUALIZADO: Centrado condicional ---
              ...
              `}
              // --- ¡NUEVO! Tooltip cuando está colapsado ---
              title={isCollapsed ? tab.label : undefined}
            >
              <tab.icon size={18} className="flex-shrink-0" />
              {/* --- ¡NUEVO! Texto condicional --- */}
              {!isCollapsed && <span className="overflow-hidden whitespace-nowrap">{tab.label}</span>}
            </button>
          );
        })}
      </nav>
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-700 space-y-2">
        {/* --- ¡NUEVO! Pasa la prop isCollapsed --- */}
        <ThemeSwitcher isCollapsed={isCollapsed} />
        <button
          onClick={onLogout}
          // --- ESTILO ACTUALIZADO: Centrado condicional ---
          className={`flex items-center w-full gap-3 px-3 py-2.5 rounded-lg text-sm font-medium
        ...
        >
          ...
          {/* --- ¡NUEVO! Texto condicional --- */}
          {!isCollapsed && <span className="overflow-hidden whitespace-nowrap">Cerrar Sesión</span>}
        </button>
      </div>
    </aside>
  );
